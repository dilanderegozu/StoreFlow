    ...
    public class TodoController : Controller
    {
        ...
        public IActionResult TodoAggreagatePriority()
        {
            var todos = _context.Todos
                ...
                .Select(y => new { y.Description, y.Priority, y.Status })
                .ToList();

            var result = todos.Any()
                ? todos.Select(t => t.Description).Aggregate((acc, desc) => acc + ", " + desc)
                : "Birincil öncelikli görev bulunamadı.";

            ViewBag.Results = result;
            ViewBag.Todos = todos.Select(t => new {
                t.Description,
                t.Priority,
                t.Status,
               
            }).ToList<dynamic>();
            ViewBag.TotalCount = _context.Todos.Count();
            ViewBag.CriticalCount = todos.Count;
